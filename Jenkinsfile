    ...
    stages {
        stage('build Unity project on spot') {
            ...
            steps {
                ...
                powershell 'Remove-Item -Path Logs -Recurse -Force'
                ...
                // script {
                //     writeJSON file: 'services-config.json', json: [
                //         'licensingServiceBaseUrl': "${env.UNITY_BUILD_SERVER_URL}",
                ...
                New-Item -Path "$Env:ProgramData\\Unity\\config" -ItemType Directory -Force
                @{
                    licensingServiceBaseUrl = "$Env:UNITY_BUILD_SERVER_URL"
                    enableEntitlementLicensing = $true
                    enableFloatingApi = $true
                    clientConnectTimeoutSec = 5
                    clientHandshakeTimeoutSec = 10
                } | ConvertTo-Json | Out-File -FilePath "$Env:ProgramData\\Unity\\config\\services-config.json" -Encoding ascii
                # Copy-Item -Path .\\services-config.json -Destination "$Env:ProgramData\\Unity\\config"

                New-Item -Path .\\iOSProj -ItemType Directory -Force
                New-Item -Path .\\Build\\iosBuild -ItemType Directory -Force

                & "$Env:ProgramFiles\\Unity\\Hub\\Editor\\2021.3.26f1\\Editor\\Unity.exe" `
                    -quit `
                    -batchmode `
                    -nographics `
                    ...
                    -adb2 -enableCacheServer | Out-String -Stream

                Write-Output "===Zipping Xcode project"
                Compress-Archive -Path iOSProj -DestinationPath iOSProj.zip
                '''
